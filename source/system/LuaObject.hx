...
class LuaObject
{
    ...
    public function new(script:String, parent:FlxState) {
	...
	public function getObjectDirectly(objectName:String, ?checkForTextsToo:Bool = true):Dynamic {
		var result:Dynamic = LuaManager.getLuaObject(objectName, checkForTextsToo);
	...
	public function setVarInArray(instance:Dynamic, variable:String, value:Dynamic):Any {
		...
		if (array.length > 1) {
			...
			if (LuaManager.variables.exists(array[0])) {
				var retVal:Dynamic = LuaManager.variables.get(array[0]);
		...
		if (LuaManager.variables.exists(variable)) {
			LuaManager.variables.set(variable, value);
			return true;
		}
	...
	public function getVarInArray(instance:Dynamic, variable:String):Any {
		...
		if (LuaManager.variables.exists(variable)) {
			var retVal:Dynamic = LuaManager.variables.get(variable);
	...
	function resetSpriteTag(tag:String) {
		if (!LuaManager.sprites.exists(tag)) return;

		var sprite:FlxSprite = LuaManager.sprites.get(tag);
		sprite.kill();
		if (sprite.active) parent.remove(sprite, true);

		sprite.destroy();
		LuaManager.sprites.remove(tag);
	}
	...
	function addAnimByIndices(obj:String, name:String, prefix:String, indices:String, framerate:Int = 24, loop:Bool = false) {
		...
		if (LuaManager.sprites.exists(obj)) {
			var sprite:FlxSprite = LuaManager.sprites.get(obj);
	...
	function cancelTimer(tag:String) {
		if (LuaManager.timers.exists(tag)) {
			var timer:FlxTimer = LuaManager.timers.get(tag);
			timer.cancel();
			timer.destroy();
			LuaManager.timers.remove(tag);
		}
	}

	function cancelTween(tag:String) {
		if (LuaManager.tweens.exists(tag)) {
			LuaManager.tweens.get(tag).cancel();
			LuaManager.tweens.get(tag).destroy();
			LuaManager.tweens.remove(tag);
		}
	}
	...
	function doTween(tag:String, vars:String, values:Dynamic, duration:Float, ease:String) {
		...
		if (tweenTarget != null) {
			LuaManager.tweens.set(tag, FlxTween.tween(tweenTarget, values, duration, {
...
//					}
	...
	private function luaInit() {
		...
		defFunction("stringStartsWith",
		function(str:String, start:String) {
			return StringTools.startsWith(str, start);
		});
		defFunction("stringEndsWith",
		function(str:String, end:String) {
			return StringTools.endsWith(str, end);
		});
		defFunction("stringSplit",
		function(str:String, split:String) {
			return str.split(split);
		});
		defFunction("stringTrim",
		function(str:String) {
		...
		defFunction("switchState",
		function (state:String) {
			FlxG.switchState(new LuaState(state));
		});

		defFunction("loadSong",
		function (json:String) {
			FlxG.switchState(new PlayState(json));
		});
		...
		function (tag:String, image:String, x:Float, y:Float) {
			...
			sprite.antialiasing = Option.antialiasing;

			LuaManager.sprites.set(tag, sprite);
			sprite.active = true;
		});
		defFunction("makeGraphic",
		function (obj:String, width:Int, height:Int, r:Int = 255, g:Int = 255, b:Int = 255, a:Int = 255) {
			var spr:FlxSprite = LuaManager.sprites.get(obj);
		...
		function (tag:String, front:Bool = false) {
			if (LuaManager.sprites.exists(tag)) {
				var sprite:FlxSprite = LuaManager.sprites.get(tag);
//				if (!Std.isOfType(parent, PlayState))
					parent.add(sprite);
//				else {
...
//				}
		...
		function(obj:String, blend:String = '') {
			var real = LuaManager.getLuaObject(obj);
			if (real != null)
			{
		...
		defFunction("getObjectOrder", function(obj:String)
		{
			...
			if (target != null) return parent.members.indexOf(target);
		...
		defFunction("doTweenColor", function(tag:String, vars:String, r:Int = 255, g:Int = 255, b:Int = 255, a:Int = 255, duration:Float, ease:String) {
			...
			if (penisExam != null) {
				...
				LuaManager.tweens.set(tag, FlxTween.color(penisExam, duration, curColor, color.setRGB(r, g, b, a), {
					ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						LuaManager.tweens.remove(tag);
						// TODO PlayState
//						if (!Std.isOfType(parent, PlayState))
		...
		defFunction("runTimer", function(tag:String, time:Float = 1, loops:Int = 1) {
			cancelTimer(tag);
			LuaManager.timers.set(tag, new FlxTimer().start(time, function(tmr:FlxTimer) {
				if(tmr.finished) LuaManager.timers.remove(tag);

				// TODO PlayState
//				if (!Std.isOfType(parent, PlayState))
					call('onTimerCompleted', [tag, tmr.loops, tmr.loopsLeft]);
//				else game.callOnLuas('onTimerCompleted', [tag, tmr.loops, tmr.loopsLeft]);
		...
		function(tag:String, text:String, width:Int, x:Float, y:Float) {
			...
			if (LuaManager.texts.exists(tag)) {
				var textRemove:FlxText = LuaManager.texts.get(tag);
				textRemove.kill();
				
				if (textRemove.active) parent.remove(textRemove, true);
				
				textRemove.destroy();
				LuaManager.texts.remove(tag);
			}

			var text:FlxText = new FlxText(x, y, text, width);
			text.active = false;
			LuaManager.texts.set(tag, text);
		});
		defFunction( "setTextFont",
		function(tag:String, newFont:String) {
			var obj:FlxText = LuaManager.texts.get(tag);
			if (obj != null) {
		...
		function(tag:String) {
			if (LuaManager.texts.exists(tag)) {
				var shit:FlxText = LuaManager.texts.get(tag);
				if (!shit.active) {
					parent.add(shit);
					shit.active = true;
		...
		function(tag:String, destroy:Bool = true) {
			if (!LuaManager.texts.exists(tag)) return;

			var textRemove:FlxText = LuaManager.texts.get(tag);
			if (destroy) textRemove.kill();

			if (textRemove.active) {
				parent.remove(textRemove, true);
		...
		function(sound:String, volume:Float = 1, ?tag:String = null) {
			if (tag != null && tag.length > 0) {
				tag = StringTools.replace(tag, '.', '');
				if (LuaManager.sounds.exists(tag)) LuaManager.sounds.get(tag).stop();
				LuaManager.sounds.set(tag, FlxG.sound.play(Paths.sound(sound), volume, false, function() {
					LuaManager.sounds.remove(tag);
					// TODO PlayState
					if (!Std.isOfType(parent, PlayState)) call('onSoundFinished', [tag]);
//					PlayState.instance.callOnLuas('onSoundFinished', [tag]);
		...
		function(tag:String) {
			if (tag != null && tag.length > 1 && LuaManager.sounds.exists(tag)) {
				LuaManager.sounds.get(tag).stop();
				LuaManager.sounds.remove(tag);
			}
		});
		...
		function(tag:String) {
			if (tag != null && tag.length > 1 && LuaManager.sounds.exists(tag)) {
				LuaManager.sounds.get(tag).pause();
			}
		});
		defFunction("resumeSound",
		function(tag:String) {
			if (tag != null && tag.length > 1 && LuaManager.sounds.exists(tag)) {
				LuaManager.sounds.get(tag).play();
			}
		});
		...
		function(tag:String, duration:Float, fromValue:Float = 0, toValue:Float = 1) {
			...
			else if (LuaManager.sounds.exists(tag)) LuaManager.sounds.get(tag).fadeIn(duration, fromValue, toValue);
		});
		...
		function(tag:String, duration:Float, toValue:Float = 0) {
			...
			else if (LuaManager.sounds.exists(tag)) LuaManager.sounds.get(tag).fadeOut(duration, toValue);
		});
		...
		function(tag:String) {
			...
			else if (LuaManager.sounds.exists(tag)) {
				var theSound:FlxSound = LuaManager.sounds.get(tag);
		...
		function(tag:String) {
			...
			else if (LuaManager.sounds.exists(tag)) return LuaManager.sounds.get(tag).volume;
			return 0;
		});
		...
		function(tag:String, value:Float) {
			...
			else if (LuaManager.sounds.exists(tag)) LuaManager.sounds.get(tag).volume = value;
		});
		defFunction("getSoundTime",
		function(tag:String) {
			if (tag != null && tag.length > 0 && LuaManager.sounds.exists(tag)) return LuaManager.sounds.get(tag).time;
			return 0;
		});
		defFunction("setSoundTime",
		function(tag:String, value:Float) {
			if (tag != null && tag.length > 0 && LuaManager.sounds.exists(tag)) {
				var theSound:FlxSound = LuaManager.sounds.get(tag);
				if (theSound != null) {
		...
		defFunction("getStrumProperty",
		function(index:Int, val:String){
			return Reflect.getProperty(PlayState.INSTANCE.strums[index], val);
		});
	}
}
